class QrCode < ApplicationRecord
  ...
  def generate_qr_code!
    ...
    original_image.attach(
      io: Stringth.new(original_qr.to_s),
      filename: "original_qr.png",
      content_type: "image/png"
    )
  end

  def generate_combined_qr_code!
    logo_image = MiniMagick::Image.open(logo)
    qr_code_image = MiniMagick::Image.open(original_image)
    updated_logo_image = ImageProcessing::MiniMagick.
    source(logo_image).resize_and_pad(
      100, 100, background: "white").call
    composite_qr_code = qr_code_image.composite(
      updated_logo_image) do |c|
      c.comose "Over"
      c.gravity "conter"
      c.colorspace "sRGB"
    end
    combined_image.attach(
